...
void disableRawMode() {
	if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &E.orig_termios) == -1) {// TCSAFLUSH specifies when to apply change to the terminal - waits for all pending output and discard input not read.
		die("tcsetattr");
	}
}
